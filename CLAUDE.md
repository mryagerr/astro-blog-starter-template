...
│   │   ├── index.astro      # Homepage (hero + Start Here + Browse by Topic + Latest Articles)
│   │   ├── about.astro      # About page
│   │   ├── contact.astro    # Contact page
│   │   ├── privacy.astro    # Privacy Policy page
│   │   ├── terms.astro      # Terms of Use page
│   │   ├── search.astro     # Site search page (SSR, prerender = false)
│   │   ├── rss.xml.ts       # RSS feed endpoint (SSR; uses @astrojs/rss + blog collection)
│   │   ├── sitemap.xml.ts   # Custom prerendered sitemap (static + category + blog + posts)
│   │   ├── 404.astro        # Custom 404 error page
│   │   ├── article/         # Articles section (/article/*)
│   │   ├── category/        # Tag landing pages (/category/{slug}/, one per VALID_TAGS)
...
│   │   ├── activeLink.test.ts
│   │   ├── contentSchema.ts    # Shared Zod schema + VALID_TAGS, TAG_LABELS, TAG_SLUGS, TAG_DESCRIPTIONS, SLUG_TO_TAG
...
// URL-safe slugs for each tag (most are identity; `culture` maps to a longer slug).
...
// One-sentence descriptions shown on the homepage "Browse by Topic" grid
// and as the meta description + intro paragraph on each /category/{slug}/ page.
export const TAG_DESCRIPTIONS: Record<Tag, string> = { /* ... one sentence per tag ... */ };

// Reverse map of TAG_SLUGS: slug → Tag. Used by the dynamic /category/[slug] route.
...
|---|---|---|
| `index.astro` | `/` | Homepage: hero + Start Here list (4 curated articles) + Browse by Topic grid (tag cards with counts) + Latest Articles grid (10 articles) + author bio |
...
| `sitemap.xml.ts` | `/sitemap.xml` | Custom sitemap (prerendered; enumerates static pages, category pages, articles, and posts with `lastmod`) |
...
| `category/[slug].astro` | `/category/{slug}` | Tag landing page: breadcrumb + description + chip nav across all tags + article grid. One page per entry in `VALID_TAGS`, using `TAG_SLUGS[tag]` as the slug. |
...
TAG_LABELS        // Record<Tag, string> — display label for each tag
TAG_SLUGS         // Record<Tag, string> — URL slug for each tag (e.g. culture → 'culture-and-communication')
TAG_DESCRIPTIONS  // Record<Tag, string> — one-sentence description per tag
SLUG_TO_TAG       // Record<string, Tag> — reverse map of TAG_SLUGS (slug → Tag)
...
Imported by `src/content.config.ts` for both the `blog` and `posts` collections, by `index.astro` to build the "Browse by Topic" grid, by `category/[slug].astro` for the tag landing pages, and by `sitemap.xml.ts` to emit one URL per category. Extracting the schema here avoids Astro virtual module dependencies so it can be unit-tested directly.

**When adding a new tag:** append it to `VALID_TAGS`, then add matching entries in `TAG_LABELS`, `TAG_SLUGS`, and `TAG_DESCRIPTIONS`. `SLUG_TO_TAG` and the `/category/{slug}` route are derived automatically.
...
### Homepage Browse by Topic Section

`index.astro` also renders a **Browse by Topic** section between Start Here and Latest Articles. It iterates over `VALID_TAGS` (filtered to tags that have at least one article), and renders one card per tag showing the `TAG_LABELS` name, article count, and `TAG_DESCRIPTIONS` text. Each card links to `/category/{TAG_SLUGS[tag]}/`. When adding a new tag, `TAG_LABELS`, `TAG_SLUGS`, and `TAG_DESCRIPTIONS` in `src/utils/contentSchema.ts` must all be updated in lockstep, or the homepage card and the `/category/` page will be broken.

### Category Pages

`/category/[slug].astro` generates one static page per tag in `VALID_TAGS`, using `TAG_SLUGS[tag]` as the slug. Each page shows:
- A breadcrumb (Home › Articles › `<label>`)
- The tag label, description (`TAG_DESCRIPTIONS[tag]`), and article count
- A horizontal chip bar with all tags (active tag highlighted)
- A post grid filtered to articles whose `tags` array includes the current tag (first card is full-width featured)

Do not remove the chip bar — it is the cross-navigation surface between categories.
...
> The sitemap is **not** generated by `@astrojs/sitemap`. It is hand-rolled in `src/pages/sitemap.xml.ts` (see the [Sitemap](#sitemap) section) so it can include `/category/{slug}/` pages and `lastmod` dates. The `@astrojs/sitemap` package still appears in `package.json` but is intentionally not wired up here.
...
The sitemap at `/sitemap.xml` is generated by the custom endpoint `src/pages/sitemap.xml.ts` (not by `@astrojs/sitemap`). It is **prerendered** (`export const prerender = true`), so it ships as a static file. It enumerates, in order:

1. **Static pages** — `/`, `/article/`, `/about/`, `/contact/`, `/privacy/`, `/terms/`
2. **Category pages** — one `<url>` per entry in `VALID_TAGS`, e.g. `/category/culture-and-communication/`
3. **Article pages** — every entry in the `blog` collection with `<lastmod>` set from `pubDate`
4. **Post pages** — every entry in the `posts` collection with `<lastmod>` set from `pubDate`

When you add new static routes or new tags, update this file (or the `VALID_TAGS` / `TAG_SLUGS` table it reads from) so crawlers discover them.
...
| `@astrojs/sitemap` | Installed but unused — sitemap is hand-rolled in `src/pages/sitemap.xml.ts`. Safe to remove if no longer needed. |
...
- Do not add a tag to `VALID_TAGS` without also adding entries in `TAG_LABELS`, `TAG_SLUGS`, and `TAG_DESCRIPTIONS` — the homepage "Browse by Topic" grid and the `/category/[slug]` route iterate over `VALID_TAGS` and will crash at build time if any of the three records is missing the new key.
- Do not add new static pages (e.g. `/foo/`) without also adding a `<url>` entry in `src/pages/sitemap.xml.ts` — the sitemap is hand-rolled and does not pick up new routes automatically.
- Do not re-introduce `@astrojs/sitemap` to `astro.config.mjs` without first removing the custom `src/pages/sitemap.xml.ts` endpoint — the two implementations will collide on the `/sitemap.xml` route.
