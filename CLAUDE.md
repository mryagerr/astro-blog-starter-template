...
Defined in `src/content.config.ts`. **Only `blog` is registered.** The schema lives in `src/utils/contentSchema.ts` (not in `content.config.ts`) so it can be unit-tested without depending on Astro's virtual modules.
...
The `blog` collection lives at `src/content/blog/` and is routed at `/article/{id}/` via `src/pages/article/[...slug].astro`. The filename becomes the URL slug.

> ⚠️ `src/content/posts/` exists on disk with three orphaned `.md` files. **It is not registered as a collection** and there is no `/posts/` route. Do not reference it from new code; do not re-add the route without explicit intent.
...
|---|---|---|---|
| `/` | `index.astro` | prerender | Hero + Start Here (4 hardcoded `START_HERE_IDS`) + Browse by Topic + Latest Articles (10) + bio |
| `/article/` | `article/index.astro` | prerender | Listing of all blog entries |
| `/article/{id}/` | `article/[...slug].astro` | prerender | Computes `readingTime`, `wordCount`, `relatedPosts`, passes them to `BlogPost.astro` |
| `/category/{slug}/` | `category/[slug].astro` | prerender | Per-tag landing page; `getStaticPaths` iterates `VALID_TAGS` and emits `TAG_SLUGS[tag]` |
...
| `relatedPosts.ts` | `findRelatedPosts(currentId, currentTags, allPosts, hrefPrefix, limit=3)` — ranks by shared-tag count, ties broken by pubDate desc |
...
- **Homepage (`index.astro`)** must keep the Start Here section (4 articles by ID in `START_HERE_IDS`) and the Browse by Topic grid (driven by `VALID_TAGS` × `TAG_SLUGS` × tag counts). Only the `blog` collection appears here.
...
- Do not re-register the `posts` collection or restore a `/posts/` route without explicit intent — both were intentionally removed.
...
- Do not put new content outside `src/content/blog/` — that is the only registered collection directory.
