...
`/search` is an SSR page (`export const prerender = false`). It reads `?q=` from the URL, filters both `blog` and `posts` collections by title/description match (case-insensitive), and returns results sorted newest-first. Results display type badge (Article / Post), date, title, and description.
...
`astro.config.mjs` includes a custom Rehype plugin `rehypeResponsiveImages` that automatically adds `loading="lazy"` and `decoding="async"` to all `<img>` elements in Markdown/MDX content. This runs on every build. Do not remove it.
