...
│   │   ├── sitemap.xml.ts   # Custom sitemap (prerender = true; generates full XML)
│   │   ├── 404.astro        # Custom 404 error page
│   │   ├── article/         # Articles section (/article/*)
│   │   ├── posts/           # Blog posts section (/posts/*)
│   │   └── category/        # Category landing pages (/category/{tag-slug}/)
│   ├── styles/
...
// URL slugs for category pages — 'culture' maps to 'culture-and-communication'
...
// Reverse lookup: URL slug → Tag (e.g., 'culture-and-communication' → 'culture')
...
type Props = CollectionEntry<'blog'>['data'] & {
  ...
  wordCount?: number;            // Raw word count (for BaseHead JSON-LD)
...
|---|---|---|
| `index.astro` | `/` | Homepage: hero + Start Here list + Browse by Topic + Latest Articles grid (10) + author bio |
...
| `sitemap.xml.ts` | `/sitemap.xml` | Custom sitemap (`prerender = true`; generates full XML with all routes) |
...
| `category/[slug].astro` | `/category/{tag-slug}/` | Category landing page; statically generated from VALID_TAGS + TAG_SLUGS |
...
Related posts score 2 pts per shared direct tag and 1 pt per shared adjacent-category tag (via `CATEGORY_ADJACENCY`). Ties break by pubDate descending, then id alphabetically for determinism. The `findRelatedPosts()` function lives in `src/utils/relatedPosts.ts`.
...
TAG_LABELS         // Record<Tag, string> — display labels (e.g., 'culture' → 'Culture & Communication')
TAG_SLUGS          // Record<Tag, string> — URL slugs (e.g., 'culture' → 'culture-and-communication')
TAG_DESCRIPTIONS   // Record<Tag, string> — full category description text
SLUG_TO_TAG        // Record<string, Tag> — reverse lookup from URL slug → Tag
...
Imported by `src/content.config.ts` for both the `blog` and `posts` collections. Extracting the schema here avoids Astro virtual module dependencies so it can be unit-tested directly. `TAG_SLUGS` and `SLUG_TO_TAG` are also used by `sitemap.xml.ts` and `category/[slug].astro`.
...
// Splits on whitespace and filters empty tokens. Returns raw word count.
...
```typescript
CATEGORY_ADJACENCY: Record<string, string[]>
// Cross-category fallback map: collection↔preparation, pipelines↔analysis, culture↔career
// Used to fill slots when a category has fewer articles than the limit.
...
// Sort order: score desc → pubDate desc → id alphabetical (deterministic).
...
`index.astro` includes a **Browse by Topic** section between Start Here and Latest Articles. It renders tag-filter buttons for all 6 categories, shows the article count per category, and links each button to `/category/{TAG_SLUGS[tag]}/`. Do not remove this section.
...
`src/pages/sitemap.xml.ts` is a **static endpoint** (`export const prerender = true`) that generates a complete XML sitemap at `/sitemap.xml` at build time. It pulls all `blog` and `posts` collection entries plus all static pages and category pages (using `TAG_SLUGS` to build `/category/{slug}/` URLs). There is no `@astrojs/sitemap` integration — the sitemap is fully custom.
...
- Do not remove the Browse by Topic section from `index.astro` — it links to category landing pages and is part of core navigation.
- Do not add `@astrojs/sitemap` back — the sitemap is now generated by `src/pages/sitemap.xml.ts` which includes category pages; the integration would conflict.
- Do not change `sitemap.xml.ts` from `prerender = true` to SSR — the sitemap must be statically built to be served correctly.
- Do not add category pages for tags outside `VALID_TAGS` — the `category/[slug].astro` route is generated from that constant via `getStaticPaths()`.
- Do not define new tag slugs inline in page files — URL slugs live in `TAG_SLUGS` in `src/utils/contentSchema.ts` so they stay in sync across sitemap, category pages, and the Browse by Topic buttons.
